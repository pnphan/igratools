...
def get_availability(station_id, download=True, download_dir=None, download_availability=False):
    """
...
def check_availability(station_id, download=True, download_dir=None, ts=False, dpy=False, mpy=False, spy=False):
    """
    Check the availability of IGRA data for a given station.
    """
    availability = []
    try:
        if download:
            availability = get_availability(station_id, download=True, download_dir=download_dir, download_availability=True)
        else:
            availability = get_availability(station_id, download=False, download_dir=download_dir, download_availability=True)

        if ts:
            print(f'\nAvailable years: {availability["available_years"]}\n')
            print(f'\n{len(availability["raw_data"])} soundings available for {station_id}\n')

        if dpy:
            print(f'\nNumber of available days per year:')
            print(availability["num_days_per_year"])

        if mpy:
            print(f'\nNumber of available months per year:')
            print(availability["num_months_per_year"])  

        if spy:
            print(f'\nNumber of available soundings per year:')
            print(availability["num_soundings_per_year"])

    except Exception as e:
        print(f"Error checking availability for station {station_id}: {e}")
        return None
...
def main():
    ...
    parser.add_argument('--da', dest='download_availability', action='store_true', 
                        help='Do not download availability data (use existing files)')
    parser.add_argument('--ts', dest='ts', action='store_true', 
                        help='Print available years and total soundings')
    parser.add_argument('--dpy', dest='dpy', action='store_true', 
                        help='Print number of available days per year')
    parser.add_argument('--mpy', dest='mpy', action='store_true', 
                        help='Print number of available months per year')
    parser.add_argument('--spy', dest='spy', action='store_true', 
                        help='Print number of available soundings per year')
    
    args = parser.parse_args()
    
    check_availability(args.stations, download=args.download, download_dir=args.download_dir, ts=args.ts, dpy=args.dpy, mpy=args.mpy, spy=args.spy)
